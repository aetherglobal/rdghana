...
- **Env (required):** `DATABASE_URI` (Neon Postgres connection string) and `PAYLOAD_SECRET`. See `.env.example`. The app will not boot without these.
...
- **Database:** Neon Postgres via `@payloadcms/db-postgres` (Drizzle). Dev uses `push: true` (schema auto-syncs — no migrations); production uses `payload migrate`.
